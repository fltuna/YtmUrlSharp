...
あんまりソフトウェアに対して責任負いたくないので、各自Buildして使ってください。

何かしら理由が無い限りは、pre-compileされたexeは配布しないです。

## 機能

- VRChat の動画再生失敗を自動検出し、ストリーム URL を抽出
- クリップボードにコピーした YouTube URL を自動検出・処理
- yt-dlp と deno の自動ダウンロード・更新
- 変換履歴の保存・復元(最大 50 件)
- ストリーム種別によるフィルター表示
...
YouTube の nsig チャレンジを解くために、yt-dlp は JavaScript エンジンを必要とします。
これが無いと **URL の抽出自体は成功するのに、その URL が HTTP 403 で弾かれる**という分かりにくい壊れ方をします。
ツール側は成功表示のまま、VRChat 側だけが再生に失敗するので、原因の切り分けができません。

そのため yt-dlp と一緒に deno を自動取得し、`--js-runtimes` で渡しています。
初回起動時に約 40MB のダウンロードが発生します。すでに deno が PATH にある場合はそれを使います。

ダウンロードしたバイナリとキャッシュは `%LocalAppData%\YtmUrlSharp` 配下にまとめており、ユーザープロファイルの他の場所は汚しません。

## UI

![desktop](./ui_desktop.png)
![vr](./ui_vr.jpeg)
